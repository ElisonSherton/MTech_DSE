...
@create date 2021-12-06 18:03:26
@modify date 2021-12-06 22:05:56
@desc [Gauss Seidel way to solve a system of linear equations Ax = b]
'''
...
def solve(input_matrix: np.array, 
    ...
    iteration_matrix = -1.0 * i_add_linv @ U
    
    # Create a container to hold all the solutions at different time-steps
    sols = []

    # Start from all 0s as the inital solution
    solution = np.zeros(nr)
    sols.append(solution)

    for it in range(iterations):
        solution = iteration_matrix @ solution + i_add_linv @ b
        sols.append(solution)
